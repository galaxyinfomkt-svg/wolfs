...
const SERVICE_PROJECT_PHOTOS: Record<string, { src: string; alt: string }[]> = {
  ...
  "default": [
    ...
    { src: "/services/hardie-plank-siding.jpg", alt: "Professional siding project by Wolf's Siding, MA" },
  ],
};
...
export async function generateMetadata({ params }: { params: Promise<{ service: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { title, description, url: `https://wolfs-siding.com/services/${slug}`, siteName: "Wolf's Siding Inc.", type: "website", images: [{ url: service.heroImage.startsWith("/") ? `https://wolfs-siding.com${service.heroImage}` : service.heroImage, width: 1200, height: 630, alt: title }] },
    twitter: {
...
export default async function ServicePage({ params }: { params: Promise<{ service: string }> }) {
  ...
  if (relatedPosts.length < 3) {
    const remaining = BLOG_POSTS.filter((p) => p.relatedService !== slug).slice(0, 3 - relatedPosts.length);
            ...
            <div>
              ...
              <div className="mb-6">
                <span className="inline-flex items-center gap-2 bg-[#E00000] text-white text-xs font-bold px-5 py-2 rounded-full tracking-wide uppercase">
                  Serving {CITIES.length}+ MA Cities &bull; 5-Star Rated &bull; HIC #218835
                </span>
              </div>
